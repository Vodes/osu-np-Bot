...
public class WindowEditCC {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					WindowEditCC window = new WindowEditCC();
//					window.initialize();
...
//	}
	 ...
	 */
	public void initialize(CustomCommand cc) {
		frmEditing = new JFrame();
		frmEditing.setResizable(false);
		frmEditing.setTitle("Editing: " + cc.getCommand());
		...
		textField.setText(cc.getCommand());
		...
		rdbtnTextResponse.setSelected(cc.getType() == CCType.Text);
		...
		rdbtntxtFileText.setSelected(cc.getType() == CCType.File);
		...
		textField_1.setText(cc.getResponse());
		...
		textField_2.setText(cc.getFile().getAbsolutePath());
		...
		btnSave.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				newcc.setEnabled(cc.isEnabled());
				newcc.respondInWhisper = cc.respondInWhisper;
